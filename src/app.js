...
var _searchDebT=null;
function dbSearch(fn){if(_searchDebT)clearTimeout(_searchDebT);_searchDebT=setTimeout(fn,120);}
...
// v5.24.0: recurring helpers. Schema: task.recur = {freq:'daily'|'weekly'|'monthly', interval:1} or null.
// On completion of an active recurring task, advance dueDate to the next future occurrence and log
// to taskHistory instead of marking done -- single row, no list clutter.
function tkAdvanceDueDate(dateStr, recur){
  if(!recur || !recur.freq || !dateStr) return dateStr;
  var d = new Date(dateStr+'T00:00:00');
  var n = Math.max(1, recur.interval||1);
  var today = new Date(); today.setHours(0,0,0,0);
  var hops = 0;
  do {
    if(recur.freq==='daily')        d.setDate(d.getDate()+n);
    else if(recur.freq==='weekly')  d.setDate(d.getDate()+7*n);
    else if(recur.freq==='monthly') d.setMonth(d.getMonth()+n);
    else break;
    hops++;
  } while(d <= today && hops < 365);
  return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0');
}
function tkRecurLabel(recur){
  if(!recur || !recur.freq) return '';
  var n = Math.max(1, recur.interval||1);
  if(n===1) return recur.freq==='daily'?'Daily':recur.freq==='weekly'?'Weekly':'Monthly';
  return 'Every '+n+' '+(recur.freq==='daily'?'days':recur.freq==='weekly'?'weeks':'months');
}
// Yields the dates within [startIso, endIso] (inclusive) on which task t should appear.
// Non-recurring: just [t.dueDate] if it falls in the window. Paused/done: nothing.
function tkOccurrencesInRange(t, startIso, endIso){
  if(!t.dueDate || t.paused || t.done) return [];
  if(!t.recur || !t.recur.freq){
    return (t.dueDate>=startIso && t.dueDate<=endIso) ? [t.dueDate] : [];
  }
  var n = Math.max(1, t.recur.interval||1);
  var d = new Date(t.dueDate+'T00:00:00');
  var end = new Date(endIso+'T00:00:00');
  var dates = [];
  var hops = 0;
  while(d <= end && hops < 366){
    var iso = d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0');
    if(iso >= startIso) dates.push(iso);
    if(t.recur.freq==='daily')        d.setDate(d.getDate()+n);
    else if(t.recur.freq==='weekly')  d.setDate(d.getDate()+7*n);
    else if(t.recur.freq==='monthly') d.setMonth(d.getMonth()+n);
    else break;
    hops++;
  }
  return dates;
...
/* v5.25.1: Tasks Phase 2 segment 2 -- swipe-to-complete on mobile.
   Touch only. Swipe a card to the right past the threshold (~45% of card
   width) and release to mark done. Card animates the rest of the way off
   and toggleTaskDone fires on the way out.
   Disambiguation: cards have touch-action:pan-y so the browser owns vertical
   scrolling; horizontal motion is ours. We confirm horizontal lock only
   after dx outruns dy by 1.5x AND |dx| > 10px. */
var _tkSwipe = null;
function tkSwipeStart(ev, taskId){
  if(ev.pointerType !== 'touch') return;
  if(!document.body.classList.contains('touch-device')) return;
  if(_tkDrag) return; // a reorder is in progress
  // Don't start a swipe when the pointer touched an interactive child.
  if(ev.target.closest('button, .tk-card-check, .tk-card-drag-handle, input, a, label, .tk-card-sub-row, .tk-card-subrow-clickable, select, textarea')) return;
  var card = document.getElementById('task-'+taskId);
  if(!card) return;
  // Skip done tasks -- swipe-to-undo would need a different threshold/UX.
  var t = tasks.find(function(x){return x.id===taskId;});
  if(!t || t.done) return;
  _tkSwipe = {
  ...
  document.addEventListener('pointermove', tkSwipeMove);
  document.addEventListener('pointerup', tkSwipeEnd);
  document.addEventListener('pointercancel', tkSwipeEnd);
}
function tkSwipeMove(ev){
  if(!_tkSwipe) return;
  if(ev.pointerId !== _tkSwipe.pointerId) return;
  var dx = ev.clientX - _tkSwipe.startX;
  var dy = ev.clientY - _tkSwipe.startY;
  if(_tkSwipe.locked === null){
    if(Math.abs(dx) > 10 && Math.abs(dx) > Math.abs(dy)*1.5){
      _tkSwipe.locked = 'horizontal';
      _tkSwipe.card.style.transition = 'none';
    } else if(Math.abs(dy) > 10){
      // Vertical scroll -- abort.
      tkSwipeCleanup(false);
      return;
    } else {
      return; // still undecided
    }
  }
  if(_tkSwipe.locked !== 'horizontal') return;
  ev.preventDefault();
  var clampedDx = Math.max(0, Math.min(dx, _tkSwipe.cardWidth));
  _tkSwipe.card.style.transform = 'translateX(' + clampedDx + 'px)';
  var pct = clampedDx / _tkSwipe.cardWidth;
  if(pct > 0.45) _tkSwipe.card.classList.add('tk-card-swipe-committing');
  else _tkSwipe.card.classList.remove('tk-card-swipe-committing');
}
function tkSwipeEnd(ev){
  if(!_tkSwipe) return;
  var s = _tkSwipe;
  if(s.locked !== 'horizontal'){ tkSwipeCleanup(false); return; }
  var dx = ev.clientX - s.startX;
  var commit = dx >= s.cardWidth * 0.45;
  s.card.style.transition = 'transform .25s cubic-bezier(.4,0,.2,1), opacity .2s';
  s.card.classList.remove('tk-card-swipe-committing');
  if(commit){
    s.card.style.transform = 'translateX(' + s.cardWidth + 'px)';
    s.card.style.opacity = '0';
    tkSwipeCleanup(true);
    setTimeout(function(){ toggleTaskDone(s.taskId); }, 220);
  } else {
    s.card.style.transform = '';
    tkSwipeCleanup(true);
  }
}
function tkSwipeCleanup(keepListeners){
  _tkSwipe = null;
  if(!keepListeners){
    document.removeEventListener('pointermove', tkSwipeMove);
    document.removeEventListener('pointerup', tkSwipeEnd);
    document.removeEventListener('pointercancel', tkSwipeEnd);
  } else {
    // Remove on the next tick so the current handler finishes cleanly
    setTimeout(function(){
      document.removeEventListener('pointermove', tkSwipeMove);
      document.removeEventListener('pointerup', tkSwipeEnd);
      document.removeEventListener('pointercancel', tkSwipeEnd);
...
var _TK_DATE_GROUPS = ['overdue','today','tomorrow','week','later','nodate','paused','done'];
var _TK_DATE_LABELS = {overdue:'Overdue', today:'Today', tomorrow:'Tomorrow', week:'This week', later:'Later', nodate:'No date', paused:'Paused', done:'Done'};
var _TK_DATE_COLORS = {overdue:'#E05252', today:'#4A8ECC', tomorrow:'#C98A1A', week:'#7A74D4', later:'#888880', nodate:'#666666', paused:'#888880', done:'#1D9E75'};
...
async function cloudPull(){
  ...
  try{
    ...
    if(!r.ok){
      ...
      throw new Error(r.status+' '+(errBody.message||'request failed'));
    }
...
/* v5.24.10: passive cloud peek -- read the file's latest commit cheaply via
   the Commits API; if the SHA changed since we last looked, fetch the file
   once to extract the device that wrote it. This lets the status indicator
   refresh "who last wrote the cloud and when" without anyone tapping Pull. */
var _cloudPeekTimer = null;
var _cloudPeekInProgress = false;
async function cloudPeekStatus(opts){
  opts = opts || {};
  if(_cloudPeekInProgress) return false;
  var token = localStorage.getItem(GH_TOKEN_KEY);
  var repo  = cloudGetRepo();
  if(!token || !repo) return false;
  // Throttle: don't peek more than once per 30s unless forced.
  var lastPeek = parseInt(localStorage.getItem(CLOUD_PEEK_LAST_KEY)||'0',10);
  if(!opts.force && Date.now()-lastPeek < 30*1000) return false;
  _cloudPeekInProgress = true;
  try{
    var path = cloudGetPath();
    var headers = {Authorization:'token '+token, Accept:'application/vnd.github.v3+json'};
    // Step 1: tiny commits-API call to detect change.
    var commitsUrl = 'https://api.github.com/repos/'+repo+'/commits?path='+encodeURIComponent(path)+'&per_page=1';
    var r = await _cloudGhFetch(commitsUrl, {headers:headers});
    if(!r.ok){ return false; }
    var commits = await r.json();
    if(!Array.isArray(commits) || !commits.length){ return false; }
    var c = commits[0];
    var commitSha = c.sha;
    var commitDate = c.commit && c.commit.author && c.commit.author.date;
    lsSet(CLOUD_PEEK_LAST_KEY, String(Date.now()));
    // If we've already processed this commit, just bump the peek timestamp.
    var lastSeen = localStorage.getItem(CLOUD_PEEK_COMMIT_KEY);
    if(lastSeen === commitSha && !opts.force){
      cloudChipRefresh();
      return true;
    }
    // Step 2: commit changed -- fetch the file to read _deviceName/_deviceId/exportedAt.
    var fileUrl = 'https://api.github.com/repos/'+repo+'/contents/'+encodeURIComponent(path);
    var r2 = await _cloudGhFetch(fileUrl, {headers:headers});
    if(!r2.ok){ return false; }
    var meta = await r2.json();
    var jsonStr = '';
    try{
      jsonStr = atob((meta.content||'').replace(/\n/g,''));
      try{ jsonStr = decodeURIComponent(escape(jsonStr)); }catch(_){}
    }catch(_){ return false; }
    var data;
    try{ data = JSON.parse(jsonStr); }catch(_){ return false; }
    var senderName = data._deviceName || 'Unknown device';
    var senderId   = data._deviceId || '?';
    var senderAt   = data.exportedAt || commitDate || '';
    lsSet(CLOUD_REMOTE_SENDER_KEY, JSON.stringify({id:senderId, name:senderName, at:senderAt}));
    lsSet(CLOUD_PEEK_COMMIT_KEY, commitSha);
    // IMPORTANT: do NOT update CLOUD_FILE_SHA_KEY here -- that would defeat the
    // push-conflict check. The cached file SHA stays as it was at our last
    // actual pull/push.
    _cloudRefreshStatus();
    cloudChipRefresh();
    var w=document.getElementById('dbw-cloud'); if(w&&typeof dbWidgetCloud==='function') dbWidgetCloud(w);
    ...
    _cloudPeekInProgress = false;
  }
}
function cloudPeekStartTimer(){
  if(_cloudPeekTimer) return;
  // Every 5 min while the tab is visible.
  _cloudPeekTimer = setInterval(function(){
    if(document.visibilityState === 'visible') cloudPeekStatus();
  }, 5*60*1000);
  // Also peek when returning to the tab (good for phone PWA after the screen wakes).
...
function cloudChipRefresh(){
  ...
  // v5.24.10: detect if the cloud has been written by another device AFTER our last sync action.
  var sender = _cloudGetRemoteSender();
  var remoteNewer = false;
  if(sender && sender.id !== getDeviceId() && sender.at){
...
function cloudChipToggle(ev){
  ...
  // v5.24.10: opportunistic peek when popover opens (throttled internally).
  cloudPeekStatus();
...
function cloudChipRenderPopover(){
  ...
  var pullTs = parseInt(localStorage.getItem(CLOUD_LAST_PULL_KEY)||'0',10);
  var pushTs = parseInt(localStorage.getItem(CLOUD_LAST_PUSH_KEY)||'0',10);
  var lastTs = Math.max(pullTs, pushTs);
  var sender = _cloudGetRemoteSender();
  var thisDev = getDeviceName();
  var senderIsUs = sender && sender.id === getDeviceId();
  // v5.24.10: highlight if cloud has data from another device newer than our last sync.
  var remoteNewer = false;
  if(sender && !senderIsUs && sender.at){
    var cloudAt = new Date(sender.at).getTime();
    if(!isNaN(cloudAt) && cloudAt > lastTs) remoteNewer = true;
  }
  ...
  var peekTs = parseInt(localStorage.getItem(CLOUD_PEEK_LAST_KEY)||'0',10);
  var peekHtml = peekTs
    ? '<div style="font-size:10px;color:var(--text3);opacity:.7;text-align:center;margin-top:8px">Checked cloud '+esc(_cloudFormatAgo(peekTs))+' · <button type="button" onclick="cloudPeekNow()" style="background:transparent;border:none;color:var(--accent);font-size:10px;font-family:var(--font);cursor:pointer;text-decoration:underline;padding:0">refresh now</button></div>'
    : '<div style="font-size:10px;color:var(--text3);opacity:.7;text-align:center;margin-top:8px"><button type="button" onclick="cloudPeekNow()" style="background:transparent;border:none;color:var(--accent);font-size:10px;font-family:var(--font);cursor:pointer;text-decoration:underline;padding:0">check cloud now</button></div>';
  pop.innerHTML =
    ...
    '<div class="cloud-popover-actions">'+
      '<button class="btn'+(remoteNewer?' primary':'')+'" onclick="cloudChipPull()" type="button">&#8595; Pull</button>'+
      '<button class="btn'+(remoteNewer?'':' primary')+'" onclick="cloudChipPush()" type="button">&#8593; Push</button>'+
    '</div>'+
    peekHtml;
}
async function cloudPeekNow(){
  var chip = document.getElementById('cloud-chip');
  if(chip) chip.classList.add('busy');
  await cloudPeekStatus({force:true});
...
function loadData(){
  ...
  cloudChipRefresh();
  cloudChipStartTimer();
  // v5.24.10: passive cloud peek so the chip stays fresh without manual pulls
  cloudPeekStartTimer();
  setTimeout(function(){ cloudPeekStatus(); }, 2000); // initial peek shortly after load
}
...
function isPaidThisCycle(item){
  ...
  if(c==='once'){
    // One-time bill: any payment ever marks it permanently paid.
    // Previously this checked inCycle(p.date), which caused the bill to flip
    // back to "unpaid" every new cycle. (Fixed v5.20.6.)
    return itemPayments.length > 0;
  }
  if(c==='quarterly'){
...
function renderWorkspacesModal(){
  ...
  content.innerHTML=workspaces.map(function(w){
    ...
    if(_wsEditId===w.id){
      return '<div class="tk-list-manage-row">'
        +'<input id="ws-edit-emoji-'+w.id+'" type="text" value="'+esc(w.emoji||'')+'" maxlength="4" title="Emoji" style="width:34px;height:28px;text-align:center;background:var(--surface2);border:0.5px solid var(--border2);border-radius:var(--radius-md);color:var(--text);font-size:14px;outline:none;flex-shrink:0">'
        +'<input id="ws-edit-color-'+w.id+'" type="color" value="'+w.color+'" title="Color" style="width:30px;height:28px;padding:1px;background:var(--surface2);border:0.5px solid var(--border2);border-radius:var(--radius-md);cursor:pointer;flex-shrink:0">'
        +'<input id="ws-edit-name-'+w.id+'" type="text" value="'+esc(w.name)+'" style="flex:1;min-width:80px;height:28px;background:var(--surface2);border:0.5px solid var(--border2);border-radius:var(--radius-md);color:var(--text);padding:0 8px;font-size:13px;font-family:var(--font);outline:none" onkeydown="if(event.key===\'Enter\')saveWorkspaceEdit(\''+w.id+'\');else if(event.key===\'Escape\')cancelWorkspaceEdit()">'
        +'<button class="icon-btn" onclick="saveWorkspaceEdit(\''+w.id+'\')" title="Save">&#10003;</button>'
        +'<button class="icon-btn" onclick="cancelWorkspaceEdit()" title="Cancel">&#10005;</button>'
        +'</div>';
    }
    var svgEdit='<svg viewBox="0 0 12 12" width="11" height="11" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M8.5 1.5l2 2-6 6H2.5v-2l6-6z"/><path d="M7 3l2 2"/></svg>';
    return '<div class="tk-list-manage-row">'
      ...
      +'<button class="icon-btn" onclick="startWorkspaceEdit(\''+w.id+'\')" title="Edit workspace">'+svgEdit+'</button>'
  ...
  if(_wsEditId){
    setTimeout(function(){var el=document.getElementById('ws-edit-name-'+_wsEditId);if(el){el.focus();el.select();}},30);
  }
}
function startWorkspaceEdit(id){_wsEditId=id;renderWorkspacesModal();}
function cancelWorkspaceEdit(){_wsEditId=null;renderWorkspacesModal();}
function saveWorkspaceEdit(id){
  var w=getWorkspace(id);if(!w){_wsEditId=null;renderWorkspacesModal();return;}
  var nameEl=document.getElementById('ws-edit-name-'+id);
  var emojiEl=document.getElementById('ws-edit-emoji-'+id);
  var colorEl=document.getElementById('ws-edit-color-'+id);
  var name=(nameEl&&nameEl.value||'').trim();
  if(!name){nameEl&&nameEl.focus();return;}
  w.name=name;
  w.emoji=(emojiEl&&emojiEl.value||'').trim();
  w.color=(colorEl&&colorEl.value||w.color);
  saveWorkspaces();
  _wsEditId=null;
  renderWorkspacesModal();
  populateTkWorkspaceSelect((document.getElementById('tk-workspace-sel')||{}).value);
...
function renderTasks(){
  ...
  if(tkGroupMode === 'by-workspace'){
    ...
    if(_TK_DATE_GROUPS.indexOf(selectedBucket) === -1) selectedBucket = 'today';
    ...
    if(!arr.length){
      var msgMap = {
        ...
        paused:   '⏸︎ No tasks paused right now.',
...
function renderTaskCard(t, opts){
  ...
  // v5.24.0: recur + paused chips in the meta row, and a pause toggle in the action row.
  var recurChip = (t.recur&&t.recur.freq)
    ? '<span class="tk-tag" style="background:#7A74D422;color:#7A74D4;border-color:#7A74D440" title="'+esc(tkRecurLabel(t.recur))+'">&#8634; '+esc(tkRecurLabel(t.recur))+'</span>'
    : '';
  var pausedChip = t.paused
    ? '<span class="tk-tag" style="background:#88888022;color:#888;border-color:#88888055">&#10074;&#10074; Paused</span>'
    : '';
  var pauseBtnTitle = t.paused?'Resume':'Pause';
  var pauseBtnGlyph = t.paused?'&#9654;':'&#10074;&#10074;';
  var pauseBtn = '<button class="tk-ibtn" data-id="'+t.id+'" onclick="toggleTaskPaused(this.dataset.id)" title="'+pauseBtnTitle+'" style="font-size:9px">'+pauseBtnGlyph+'</button>';
  var actionsHtml=t.done?'':
    ...
    '</div>';
  var cardCls='tk-card'+(t.done?' tk-card-done':'')+(t.paused?' tk-card-paused':'');
  // v5.21.2: subtle workspace tint on the card background + border so you can
  // tell Personal from Work at a glance without parsing the meta row.
  var cardStyle='background:'+wsCol+'14;border-color:'+wsCol+'40;'+(t.paused?'opacity:.6;':'');
  ...
  +'</button>';
  // v5.25.1: swipe-to-complete -- pointerdown on the card body starts a swipe
  // (touch only; the handler aborts on mouse pointer and on interactive children).
  var swipeAttr = t.done ? '' : ' onpointerdown="tkSwipeStart(event,\''+t.id+'\')"';
  return '<div class="'+cardCls+'" id="task-'+t.id+'" style="'+cardStyle+'"'+swipeAttr+'>'+
    '<div class="tk-card-top">'+
      ...
      '</div>'+
    '</div>'+
    '<div class="tk-card-meta">'+dueHtml+wsChip+recurChip+pausedChip+tagsHtml+'</div>'+
    subtaskBar+
    notesHtml+
    actionsHtml+
  '</div>';
...
function toggleTaskDone(id){
  ...
  // v5.24.0: recurring & active & not paused -> advance dueDate, log to history, stay active.
  if(!t.done && !t.paused && t.recur && t.recur.freq && t.dueDate){
    ...
    if(taskHistory.length > 500) taskHistory.length = 500;
    t.dueDate = tkAdvanceDueDate(oldDue, t.recur);
    saveTasks(); saveTaskHistory(); renderTasks();
    toast('&#10003; '+esc(t.title)+' &#8594; next '+t.dueDate);
    return;
  }
  t.done=!t.done;
...
function toggleTaskPaused(id){
  const t=tasks.find(t=>t.id===id);if(!t)return;
  t.paused=!t.paused;
  saveTasks();renderTasks();
  toast(t.paused?'&#10074;&#10074; Paused: '+t.title:'&#9654; Resumed: '+t.title);
}
...
function openTaskModal(id){
  ...
  // v5.24.0: recur + paused fields
  var recurFreq=(t&&t.recur&&t.recur.freq)||'';
  var recurInterval=(t&&t.recur&&t.recur.interval)||1;
  var rfSel=document.getElementById('tk-recur-freq');
  var rfInt=document.getElementById('tk-recur-interval');
  var rfRow=document.getElementById('tk-recur-interval-row');
  var pauseCb=document.getElementById('tk-paused');
  if(rfSel) rfSel.value=recurFreq;
  if(rfInt) rfInt.value=recurInterval;
  if(rfRow) rfRow.style.display=recurFreq?'flex':'none';
  if(pauseCb) pauseCb.checked=!!(t&&t.paused);
...
function saveTask(){
  ...
  // v5.24.0: recur + paused
  var rfSel=document.getElementById('tk-recur-freq');
  var rfInt=document.getElementById('tk-recur-interval');
  var pauseCb=document.getElementById('tk-paused');
  var recurFreq=rfSel?rfSel.value:'';
  var recurInterval=rfInt?Math.max(1,parseInt(rfInt.value,10)||1):1;
  var recur=recurFreq?{freq:recurFreq, interval:recurInterval}:null;
  var dueIso=document.getElementById('tk-due').value||null;
  // Recurring tasks need a dueDate -- default to today.
  if(recur && !dueIso) dueIso = todayStr();
  const obj={
    ...
    recur:recur,
    paused:pauseCb?pauseCb.checked:false,
  };
...
function calGetEvents(){
  ...
  // Tasks with dueDate. v5.24.0: recurring tasks expand to all occurrences in the visible month.
  const monthStartIso = `${calYear}-${String(calMonth+1).padStart(2,'0')}-01`;
  const monthEndDay = new Date(calYear, calMonth+1, 0).getDate();
  const monthEndIso = `${calYear}-${String(calMonth+1).padStart(2,'0')}-${String(monthEndDay).padStart(2,'0')}`;
  tasks.filter(t=>!t.done&&!t.paused&&t.dueDate).forEach(t=>{
    const dates = (t.recur&&t.recur.freq)
      ? tkOccurrencesInRange(t, monthStartIso, monthEndIso)
      : [t.dueDate];
    dates.forEach(function(dStr){
      const d=new Date(dStr+'T00:00:00'); d.setHours(0,0,0,0);
      const overdue=d<now;
      const recurLabel = (t.recur&&t.recur.freq) ? ' · '+tkRecurLabel(t.recur) : '';
      addEv(dStr,{type:'task',name:t.title,meta:(t.priority&&t.priority!=='none'?`${t.priority} priority`:'')+recurLabel,color:'#9B7FE8',overdue,taskId:t.id});
    });
  });
...
const DB_WIDGETS = [
  ...
  { id:'cloud',     cols:1, render: dbWidgetCloud       },
...
/* Widget: Cloud sync (v5.24.3) -- one-tap push/pull from the dashboard */
function dbWidgetCloud(el){
  dbCard(el,{icon:'&#9729;',title:'Cloud sync',headColor:'rgba(74,142,204,.08)'});
  var body=document.getElementById('dbwb-cloud');
  if(!body) return;
  var token = localStorage.getItem(GH_TOKEN_KEY);
  var repo  = cloudGetRepo();
  if(!token || !repo){
    body.innerHTML='<div class="db-empty" style="text-align:center;padding:14px 0">'+
      '<div style="margin-bottom:6px">Cloud sync not configured</div>'+
      '<button class="db-link" onclick="openSettingsModal()">Set GitHub token + repo in Settings &rarr;</button>'+
      '</div>';
    return;
  }
  var pullTs = parseInt(localStorage.getItem(CLOUD_LAST_PULL_KEY)||'0',10);
...
async function dbCloudPull(){
  if(!confirm('Pull from cloud will replace your local data with the cloud version. Continue?')) return;
  await cloudPull();
  var w=document.getElementById('dbw-cloud'); if(w) dbWidgetCloud(w);
}
async function dbCloudPush(){
  await cloudPush();
  var w=document.getElementById('dbw-cloud'); if(w) dbWidgetCloud(w);
}
